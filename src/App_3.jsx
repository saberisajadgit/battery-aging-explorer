...
function InteractiveOCVChart({ ocvData, soc, onSocChange, Q0 }) {
  ...
  // Filter to valid full-cell points only
  const validPts = ocvData.filter(d => d.fullCell != null && d.q <= Q0 * 1.05);
  const validPristine = ocvData.filter(d => d.pristine != null && d.q <= Q0 * 1.05);
  ...
  return (
    <div style={{ position: "relative", userSelect: "none" }}>
      ...
      >
        ...
        {/* Tooltip bubble */}
        {nearestCurve && (() => {
          const bw = 148, bh = nearestP?.pristine ? 64 : 44, br = 5;
          const bx = Math.min(dotX + 12, ml + pw - bw - 4);
          const by = Math.max(mt + 4, dotY - bh - 8);
          return (
            <g>
              <rect x={bx} y={by} width={bw} height={bh} rx={br}
                fill="#0a1628" stroke={C.border} strokeWidth="1" />
              <text x={bx+10} y={by+16} fill={C.muted} fontSize="9" fontFamily="monospace">
                Q = {Math.round(dotQ)} mAh · SOC = {soc}%
              </text>
              {nearestP?.pristine && (
                <text x={bx+10} y={by+32} fill={C.muted} fontSize="9" fontFamily="monospace">
                  Pristine: <tspan fill={C.muted}>{nearestP.pristine.toFixed(4)} V</tspan>
                </text>
              )}
              <text x={bx+10} y={by+(nearestP?.pristine ? 48 : 32)} fill={C.teal} fontSize="9" fontFamily="monospace">
                Aged:{'    '}<tspan fill={C.teal}>{dotV.toFixed(4)} V</tspan>
              </text>
            </g>
          );
        })()}
        ...
        {/* Legend */}
        <line x1={ml+10} x2={ml+28} y1={mt+ph-10} y2={mt+ph-10} stroke={C.teal} strokeWidth="2.5" />
        <text x={ml+32} y={mt+ph-7} fill={C.muted} fontSize="8" fontFamily="monospace">Aged</text>
        <line x1={ml+72} x2={ml+90} y1={mt+ph-10} y2={mt+ph-10} stroke={C.muted} strokeWidth="1.5" strokeDasharray="5 3" />
        <text x={ml+94} y={mt+ph-7} fill={C.muted} fontSize="8" fontFamily="monospace">Pristine</text>
        <circle cx={ml+160} cy={mt+ph-10} r="4" fill={C.amber} />
        <text x={ml+167} y={mt+ph-7} fill={C.muted} fontSize="8" fontFamily="monospace">← drag me</text>
      </svg>
    </div>
  );
}
